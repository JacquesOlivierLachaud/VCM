...
echo "#args=${nb}"
echo " args=${args}"
if test ${nb} -lt 6; then 
    echo "Usage: $0 estimator shape kernel R r alpha"
    exit 0
fi
estimator=$1
shape=$2
...
while test `echo "${h} >= ${h_end}"|bc -l` -eq 1; do
    ...
    file="${estimator}/${kernel}-${alpha}"
    if test ! -d ${file}; then mkdir ${file}; fi
    file="${estimator}/${kernel}-${alpha}/${shape}"
    toExport=""
    ...
    echo "#      gridstep  =${h}"
    echo "#"
    echo "./implicitShape3NormalEstimation -p ${polynomial} -o ${file} -a ${min_bbox} -A ${max_bbox} -e ${estimator} -R ${bigR} -r ${smallR} -t 2 -E 1 ${toExport} --alpha ${alpha} -k ${kernel} -g ${h}"
    ./implicitShape3NormalEstimation -p ${polynomial} -o ${file} -a ${min_bbox} -A ${max_bbox} -e ${estimator} -R ${bigR} -r ${smallR} -t 2 -E 1 ${toExport} --alpha ${alpha} -k ${kernel} -g ${h}
    echo "------------------------------------------------------------------"
    echo
    echo
